 ...
 */
public class JiraClient implements ClientConstants {
	...
	class JiraTokenCommand {
		...
		public Object execute() throws Exception {
			Object issue;
			try {
				MyLogger.getLogger(JiraClient.class).trace("Conducting Action");
				issue = action.accessJiraAndReturn();
				MyLogger.getLogger(JiraClient.class).trace("Conducting Action Done!");
			} catch (RemotePermissionException e) {
				MyLogger.getLogger(JiraClient.class).error(e);
			} catch (RemoteAuthenticationException e) {
				MyLogger.getLogger(JiraClient.class).error(e);
			} catch (com.atlassian.jira.rpc.exception.RemoteException e) {
				MyLogger.getLogger(JiraClient.class).warn(e);
				renewToken();
	...
	public JiraClient() throws Exception {
	...
	public static void main(String[] args) throws Exception {
		MyLogger.getLogger(JiraClient.class).debug("Running test SOAP client...");
...
//		client.test("LLU-1235");
		...
		MyLogger.getLogger(JiraClient.class).debug("Closing test SOAP client...");
	}
	...
	private String getToken() {
		MyLogger.getLogger(JiraClient.class).debug("Getting Token");
		if (token == null) {
			MyLogger.getLogger(JiraClient.class).trace("Syncing Token Block");
			synchronized (this) {
				MyLogger.getLogger(JiraClient.class).trace("Syncing Token Block has been synced");
				if (token == null) {
					try {
						renewToken();
					} catch (Exception e) {
				...
				MyLogger.getLogger(JiraClient.class).trace("Block Synced Done!");
			}
		}
		MyLogger.getLogger(JiraClient.class).trace("Getting Token Done!");
		return token;
	}

	private void renewToken() {
		MyLogger.getLogger(JiraClient.class).debug("Renewing Token");
		try {
		...
		MyLogger.getLogger(JiraClient.class).trace("Renewing Token Done!");
	}

	public RemoteIssue getJira(final String jiraName) throws Exception {
		MyLogger.getLogger(JiraClient.class).debug("Getting Jira");
		...
		MyLogger.getLogger(JiraClient.class).trace("Getting Jira Done!");
		return (RemoteIssue) command.execute();
	}

	public RemoteVersion getFixVersion(final String fixName) throws Exception {
		MyLogger.getLogger(JiraClient.class).debug("Getting FixVersion");
