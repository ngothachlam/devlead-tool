...
public class JiraTableModel extends MyTableModel {
   ...
   public Color getColor(Object value, int row, ColumnType column, int colNo) {
      ...
      if (log.isDebugEnabled())
         log.debug("The row getting color for is " + row + " (col: " + column + " with index " + colNo + ") ");
      if (colNo == 0) {
      ...
      case Resolution:
         ...
         if (!isEmptyString(stringValue)) {
            BoardStatusValue boardStatus = (BoardStatusValue) boardModel.getValueAt(ColumnType.BoardStatus, jiraRowInBoardModel);
   ...
   boolean isJiraNumberOk(Object boardValue, Object jiraValue) {
      if (log.isDebugEnabled())
         log.debug("... We are trying to check if either the board or jira has numberical or string values (boardValue: " + boardValue + ", jiraValue: "
               + jiraValue + ")");
