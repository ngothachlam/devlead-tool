	...
	public static boolean displayURL(String url) {
		...
		try {
			...
			if (windows) {
				...
				// 0,
				// it worked, otherwise we need to start the browser. cmd = 'netscape -remote openURL
				...
				try {
					...
					if (exitCode != 0) {
						// Command failed, start up the browser cmd = 'netscape http://www.javaworld.com' cmd =
						// UNIX_PATH + " " + url;
