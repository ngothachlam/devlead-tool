...
class SprintCacheListenerImpl implements SprintCacheListener {
   private static final Logger log = MyLogger.getLogger(SprintCacheListenerImpl.class);
   ...
   public void notify(SprintCacheNotification notification) {
      ...
      case REQUIREDFIELDMISSING:
         ...
         break;
      case ADDINGTOCACHE:
         log.debug("ADDINGTOCACHE creating dialog");
         dialog = new ProgressDialog(parent, "running...", "note", 0, true);
         log.debug("done creating dialog");
         break;
      case ADDEDTOCACHE:
         log.debug("ADDEDTOCACHE is null?");
         if (dialog != null) {
            log.debug("not null");
            dialog.setCompleteWithDelay(100);
         }
...
class SprintCreationSourceImpl implements SprintCreationSource {
   ...
   public void setSources(JTextField nameTextField, JXDatePicker startDatePicker, JXDatePicker endDatePicker, JTextField lengthTextField, JTextArea noteTextArea) {
